...
const isStayNight = (reservation, day) => {
...
const evaluateLateCheckoutAlerts = (reservations, todayKey) => {
    return reservations
...
const evaluateDirtyRoomAlerts = (rooms, tasks) => {
...
const evaluatePaymentPendingAlerts = (reservations, todayKey) => {
    return reservations
        .filter((reservation) => {
...
const evaluateOverbookingAlerts = (reservations, rooms) => {
...
const sortAlerts = (alerts) => alerts.sort((a, b) => (b.updatedAt ?? b.createdAt).localeCompare(a.updatedAt ?? a.createdAt));
export const syncAlertsForHotel = async (hotelCode) => {
    ...
    const existingActive = new Map(relevantAlerts
...
export const acknowledgeAlert = async (hotelCode, alertId, acknowledgedBy) => {
