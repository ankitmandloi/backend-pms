...
export const listMenuItems = async (hotelCode) => {
...
export const createMenuItem = async (input) => {
    ...
    const items = await menuTable.getAll();
    const duplicate = items.find((item) => item.hotelCode === input.hotelCode && item.name.toLowerCase() === input.name.toLowerCase());
    if (duplicate) {
...
export const updateMenuItem = async (id, input) => {
...
const buildOrderItems = async (hotelCode, items) => {
    if (!items.length) {
...
const computeOrderTotal = (items) => Number(items.reduce((sum, item) => sum + item.totalPrice, 0).toFixed(2));
const createKot = async (order) => {
    const kot = {
        orderId: order.id,
...
export const listOrders = async (hotelCode) => {
...
export const postOrderToRoom = async (input) => {
    const order = await ensureOrder(input.orderId);
...
export const getKotQueue = async (hotelCode) => {
    const tickets = await kitchenTable.getAll();
