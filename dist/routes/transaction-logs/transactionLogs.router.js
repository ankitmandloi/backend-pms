...
router.post('/', authenticate, async (req, res, next) => {
    try {
        ...
        const { eventType, entityType, entityId, description, metadata, previousState, newState, } = req.body;
...
router.get('/', authenticate, async (req, res, next) => {
    try {
        ...
        const hotelCode = req.context?.hotelCode || req.user?.hotelCode || '';
        const transactionLogsTable = db.transactionLogs;
        const { eventType, entityType, entityId, userId, startDate, endDate, limit = '100', } = req.query;
        let logs = await transactionLogsTable.getAll();
        // Filter by hotel
        logs = logs.filter((log) => log.hotelId === hotelId && log.hotelCode === hotelCode);
        // Apply filters
        if (eventType) {
...
router.get('/entity/logs', authenticate, async (req, res, next) => {
    try {
        ...
        let logs = await transactionLogsTable.getAll();
        logs = logs.filter((log) => log.hotelId === hotelId &&
...
//# sourceMappingURL=transactionLogs.router.js.map