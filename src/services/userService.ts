...
export const toUserPayload = (record: UserRecord): UserPayload => ({
  id: record.id,
...
export const findByHotelCodeAndUsername = async (hotelCode: string, username: string) => {
  ...
  const match = records.find(
    (record) => record.hotelCode.toLowerCase() === hotelCode.toLowerCase() && record.username.toLowerCase() === username.toLowerCase()
  );
