...
const sanitizeAmenities = (amenities) => amenities?.map((amenity) => amenity.trim()).filter(Boolean) ?? [];
const DEFAULT_ROOM_AMENITIES = [];
const DEFAULT_ROOMTYPE_AMENITIES = [];
...
const ensurePropertyProfile = async () => {
...
export const upsertPropertyProfile = async (input) => {
...
const ensureRoomType = async (hotelCode, roomTypeId) => {
...
const ensureFloorForHotel = async (hotelCode, floorId) => {
    if (!floorId) {
        return undefined;
    }
...
const assertValidRoomStatusTransition = (current, next) => {
    if (current === next) {
        return;
    }
...
export const updateRoomType = async (id, input) => {
    const profile = await ensurePropertyProfile();
    ...
    if (input.shortCode) {
        ...
        const duplicateCode = roomTypes.find((record) => record.id !== id && normalizeCode(record.shortCode) === normalizedCode);
        if (duplicateCode) {
            throw new HttpError(409, "Room type short code already exists");
        }
    }
...
export const deleteRoomType = async (id) => {
    ...
    const roomsUsingType = rooms.filter((room) => room.hotelCode === profile.hotelCode &&
...
const ensureUniqueRoomNumber = (rooms, number, excludeId) => {
...
export const updateFloor = async (id, input) => {
    const profile = await ensurePropertyProfile();
...
export const getTaxConfig = async () => {
    ...
    const match = records.find((record) => record.hotelCode === profile.hotelCode);
    if (!match) {
        return null;
    }
    return {
        ...
        serviceChargeEnabled: typeof match.serviceChargeEnabled === "boolean" ? match.serviceChargeEnabled : DEFAULT_TAX_VALUES.serviceChargeEnabled,
        serviceChargePercentage: typeof match.serviceChargePercentage === "number" ? match.serviceChargePercentage : DEFAULT_TAX_VALUES.serviceChargePercentage,
        luxuryTaxEnabled: typeof match.luxuryTaxEnabled === "boolean" ? match.luxuryTaxEnabled : DEFAULT_TAX_VALUES.luxuryTaxEnabled,
        luxuryTaxPercentage: typeof match.luxuryTaxPercentage === "number" ? match.luxuryTaxPercentage : DEFAULT_TAX_VALUES.luxuryTaxPercentage
    };
};
export const upsertTaxConfig = async (input) => {
    const profile = await ensurePropertyProfile();
    const timestamp = now();
