...
export const nightAuditService = {
     ...
     */
    async getAudits(hotelId, hotelCode) {
     ...
     */
    async getLatestAudit(hotelId, hotelCode) {
        const audits = await nightAuditsTable.find({ hotelId, hotelCode });
        if (audits.length === 0)
            return null;
     ...
     */
    async isAuditRequired(hotelId, hotelCode) {
     ...
     */
    async startAudit(hotelId, hotelCode, userId, businessDate) {
     ...
     */
    async executeAuditSteps(auditId, hotelId, hotelCode) {
        ...
        try {
            ...
            if (openShifts.length > 0) {
                ...
                await this.updateStepStatus(auditId, "VALIDATE_SHIFT_CLOSURE", "FAILED", `${openShifts.length} open shifts detected`);
            }
            else {
        ...
        try {
            ...
            await this.updateStepStatus(auditId, "POST_ROOM_REVENUE", "COMPLETED", `Posted revenue for ${totalRoomsOccupied} rooms, total: ₹${totalRevenuePosted}`);
        }
        catch (error) {
        ...
        try {
            ...
            for (const reservation of confirmedReservations) {
                // Mark as no-show (cancelled in this simplified version)
                reservation.status = "CANCELLED";
                reservation.notes = (reservation.notes || "") + " [NO-SHOW]";
                reservation.updatedAt = now();
                await reservationsTable.update(reservation.id, reservation);
                noShowsProcessed++;
            }
            await this.updateStepStatus(auditId, "PROCESS_NO_SHOWS", "COMPLETED", `Processed ${noShowsProcessed} no-shows`);
        }
        catch (error) {
        ...
        try {
            ...
            await this.updateStepStatus(auditId, "UPDATE_ROOM_STATUS", "COMPLETED", `Updated ${roomStatusUpdates} room statuses`);
        }
        catch (error) {
        ...
        try {
            ...
            await this.updateStepStatus(auditId, "GENERATE_REPORTS", "COMPLETED", `Generated ${reportsGenerated.length} reports`);
        }
        catch (error) {
        ...
        try {
            ...
            await this.updateStepStatus(auditId, "ROLLOVER_DATE", "COMPLETED", "Business date rolled over successfully");
        }
        catch (error) {
     ...
     * Update step status
     */
    async updateStepStatus(auditId, stepType, status, message) {
        const audit = await nightAuditsTable.findById(auditId);
        if (!audit)
            return;
     ...
     */
    async markAuditFailed(auditId, errorMessage) {
        const audit = await nightAuditsTable.findById(auditId);
        if (audit) {
            audit.status = "FAILED";
     ...
     */
    async retryAudit(auditId, hotelId, userId) {
...
//# sourceMappingURL=nightAuditService.js.map