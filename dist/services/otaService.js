...
import { OTAAdapterRegistry } from "./adapters/index.js";
...
export const otaService = {
     ...
     */
    async createChannel(hotelId, hotelCode, input) {
     ...
     */
    async updateChannel(id, hotelId, updates) {
     ...
     * Delete OTA channel
     */
    async deleteChannel(id, hotelId) {
        const channel = await this.getChannelById(id, hotelId);
     ...
     */
    async testConnection(id, hotelId) {
        const channel = await this.getChannelById(id, hotelId);
        try {
     ...
     */
    async syncRates(channelId, hotelId, dateRange) {
     ...
     */
    async syncInventory(channelId, hotelId, dateRange) {
        const channel = await this.getChannelById(channelId, hotelId);
        if (channel.status !== "ACTIVE") {
            throw new HttpError(400, "Channel is not active");
        }
        try {
            ...
            for (const date of dates) {
                for (const mapping of channel.mappings) {
                    ...
                    const roomsOfType = rooms.filter(r => r.type === mapping.roomTypeId);
     ...
     */
    async importReservation(channelId, hotelId, otaReservationData) {
        const channel = await this.getChannelById(channelId, hotelId);
        ...
        const guestData = otaReservationData.guest;
        const reservationData = otaReservationData.reservation;
     ...
     * Get sync logs
     */
    async getSyncLogs(hotelId, hotelCode, filters) {
     ...
     */
    async getImportedReservations(hotelId, hotelCode, provider) {
