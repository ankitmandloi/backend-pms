...
const transitionRoomStatus = async (room, status) => {
...
export const updateHousekeepingStatus = async (input) => {
    const room = await ensureRoom(input.roomId);
    const task = await ensureTask(room);
