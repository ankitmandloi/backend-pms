 ...
 */
nightAuditRouter.get("/", authorize(["ADMIN", "MANAGER", "ACCOUNTING"]), asyncHandler(async (req, res) => {
 ...
 * Accessible by: ADMIN, MANAGER, ACCOUNTING
 */
nightAuditRouter.get("/latest", authorize(["ADMIN", "MANAGER", "ACCOUNTING"]), asyncHandler(async (req, res) => {
    const { hotelId, hotelCode } = req.user;
 ...
 * Accessible by: ADMIN, MANAGER, ACCOUNTING
 */
nightAuditRouter.get("/check-required", authorize(["ADMIN", "MANAGER", "ACCOUNTING"]), asyncHandler(async (req, res) => {
    const { hotelId, hotelCode } = req.user;
 ...
 * Accessible by: ADMIN, MANAGER, ACCOUNTING
 */
nightAuditRouter.get("/:id", authorize(["ADMIN", "MANAGER", "ACCOUNTING"]), asyncHandler(async (req, res) => {
 ...
 */
nightAuditRouter.post("/start", authorize(["ADMIN", "MANAGER"]), asyncHandler(async (req, res) => {
    const { hotelId, hotelCode, userId, sub } = req.user;
    const { businessDate } = req.body;
    const audit = await nightAuditService.startAudit(hotelId, hotelCode, userId || sub, businessDate);
    res.json({ success: true, data: audit });
}));
/**
 ...
 */
nightAuditRouter.post("/:id/retry", authorize(["ADMIN", "MANAGER"]), asyncHandler(async (req, res) => {
    const { hotelId, userId, sub } = req.user;
    const { id } = req.params;
    const audit = await nightAuditService.retryAudit(id, hotelId, userId || sub);
    res.json({ success: true, data: audit });
}));
//# sourceMappingURL=nightAudit.router.js.map