 ...
 */
otaRouter.get("/channels", asyncHandler(async (req, res) => {
 ...
 */
otaRouter.get("/channels/:id", asyncHandler(async (req, res) => {
 ...
 */
otaRouter.post("/channels", asyncHandler(async (req, res) => {
    const { hotelId, hotelCode } = req.user;
 ...
 * Update OTA channel
 */
otaRouter.put("/channels/:id", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
 ...
 * Delete OTA channel
 */
otaRouter.delete("/channels/:id", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
 ...
 * Test OTA connection
 */
otaRouter.post("/channels/:id/test", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
 ...
 * Sync rates to OTA
 */
otaRouter.post("/channels/:id/sync-rates", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
 ...
 */
otaRouter.post("/channels/:id/sync-inventory", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
    const { from, to } = req.body;
 ...
 */
otaRouter.post("/channels/:id/import-reservation", asyncHandler(async (req, res) => {
    const { hotelId } = req.user;
    const { id } = req.params;
 ...
 * Get sync logs
 */
otaRouter.get("/sync-logs", asyncHandler(async (req, res) => {
    const { hotelId, hotelCode } = req.user;
 ...
 */
otaRouter.get("/imported-reservations", asyncHandler(async (req, res) => {
    const { hotelId, hotelCode } = req.user;
    const { provider } = req.query;
    const imports = await otaService.getImportedReservations(hotelId, hotelCode, provider);
    res.json({ success: true, data: imports });
}));
//# sourceMappingURL=ota.router.js.map