...
const sum = (values) => Number(values.reduce((acc, value) => acc + value, 0).toFixed(2));
const isActiveReservation = (reservation) => reservation.status !== "CANCELLED";
const isStayNight = (reservation, target) => {
...
export const getDashboardSummary = async (hotelCode, date) => {
    ...
    const relevantReservations = reservations.filter((reservation) => reservation.hotelCode === hotelCode && isActiveReservation(reservation));
    ...
    const otherCharges = charges.filter((entry) => entry.charge.type === "ADJUSTMENT" && getChargeDateKey(entry.charge) === dateKey);
    ...
    const pendingTasks = tasks.filter((task) => task.hotelCode === hotelCode && ["PENDING", "IN_PROGRESS", "INSPECT"].includes(task.status));
...
export const getDailyReport = async (hotelCode, date) => {
    const targetDate = parseDateInput(date);
    const dateKey = toDateKey(targetDate);
    ...
    const stayovers = relevantReservations.filter((reservation) => isStayNight(reservation, targetDate) && reservation.arrivalDate !== dateKey);
    return {
        date: dateKey,
...
export const getRevenueReport = async (hotelCode, params = {}) => {
    ...
    const daily = dateKeys.map((key) => {
        ...
        const totalRevenue = roundCurrency(roomRevenue + addonRevenue + otherRevenue);
        const paymentsCollected = sum(relevantPayments
